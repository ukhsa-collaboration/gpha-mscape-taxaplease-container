...
def test_root_returns_itself_as_parent():
    ...
    assert result.captured.out.strip() == b"1"
...
def test_taxid_2000_is_streptosporangium():
    ...
    assert json.loads(result.captured.out.decode("utf-8")).get("name") == "Streptosporangium"
...
def test_parent_taxa():
    ...
    assert json.loads(result.captured.out.decode("utf-8")) == 85012
...
def test_get_genus_taxid():
    ...
    assert json.loads(result.captured.out.decode("utf-8")) == 561
...
def test_common_parent_record_distant_taxa():
    ...
    assert json.loads(result.captured.out.decode("utf-8")).get("name") == "Eukaryota"
...
def test_common_parent_record_close_taxa():
    ...
    assert json.loads(result.captured.out.decode("utf-8")).get("name") == "Enterobacteriaceae"
...
def test_levels_between_close_taxa():
    ...
    assert json.loads(result.captured.out.decode("utf-8")) == {
...
def test_is_virus_fail():
    ...
    assert not json.loads(result.captured.out.decode("utf-8"))
